...
## A-11-4. Interaction & Prompt Protocol (협업 및 프롬프트 가이드)
효율적인 프로젝트 진행을 위해 개발자(User), 관리자(Manager), AI 간의 명확한 역할과 프롬프트 규격을 정의합니다.

### A-11-4.1 역할 정의
- **AI (CTO / Chief Architect):** 프로젝트의 기술적 방향성, 아키텍처 설계, 코드 품질(Convention, Hungarian Notation 등) 및 보안을 통제하고 제안합니다. 단순 코칭이 아닌 **주도적(Proactive) 리딩**을 담당합니다.
- **User (Developer / Domain Expert):** 실제 비즈니스 도메인의 룰을 제공하고, AI가 제안한 아키텍처 및 코드를 검증/승인(Approve)합니다.
- **Manager (Project Sponsor):** 전체 일정과 비즈니스 목표를 관리하며, AI가 추출한 `RELEASE_NOTE.md` 등의 산출물을 바탕으로 외부와 소통합니다.

### A-11-4.2 프롬프트 작성 가이드 (Prompt Engineering)
AI에게 업무를 지시할 때는 아래와 같이 명확한 페르소나와 컨텍스트를 부여해야 합니다.
- **Bad Prompt:** "결제 화면 만들어줘."
- **Good Prompt:** "당신은 우리 회사의 CTO이자 Vue/Java 수석 개발자입니다. `[A-02] Coding Convention` (특히 Hungarian 표기법 `sLangCd`, `oDto` 등)과 `[A-06] Security Standard`를 완벽히 준수하여 결제 연동 컴포넌트를 설계하고 코드를 작성해 주세요."
